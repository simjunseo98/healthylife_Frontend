...
type ExerciseOptionProps = {
    onSelect: (exercise_name: string, body_part: string) => void
}
...
const SERVER_PART_KEYS: Record<string, string> = {
    bottom: '하체',
    back: '등',
    chest: '가슴',
    shoulder: '어깨',
        ...
        <div className="flex flex-col gap-6 w-full max-w-md mx-auto p-4">
            {!selectedPart ? (
                <div className="flex flex-col gap-4">
                    <p className="text-emerald-900 font-bold text-3xl text-center mb-3">
                        운동 부위를 선택하세요
                    </p>
                    <div className="flex gap-6 justify-center flex-wrap">
                        {Object.keys(EXERCISE_DATA).map((partKey) => (
                            <button
                                ...
                                className="text-white text-base sm:text-lg md:text-xl px-4 py-2 sm:px-6 sm:py-3 md:px-8 md:py-3 rounded-xl bg-emerald-500 hover:bg-sky-400 transition w-full text-center"
                            >
                                {PART_LABELS[partKey]}
                            </button>
                        ))}
                ...
                <div className="flex flex-col gap-4">
                    <p className="text-emerald-900 text-center text-3xl">
                    ...
                    <div className="flex flex-wrap gap-3 justify-center">
                        {EXERCISE_DATA[selectedPart].map((exercise) => (
                            <button
                                key={exercise}
                                onClick={() => onSelect(exercise, SERVER_PART_KEYS[selectedPart])}
                                className="text-white py-3 px-6 rounded-lg bg-emerald-500 hover:bg-sky-400 transition whitespace-nowrap"
                            >
                                {exercise}
                            </button>
                        ))}
